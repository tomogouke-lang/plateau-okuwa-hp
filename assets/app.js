(function(){
  ...
  document.title=(view==="guide"?"ご利用案内":"事業所紹介")+"｜"+store.name;
  const tel=store.facts.phone.replace(/-/g,"");
  const escapeHtml=value=>String(value).replace(/[&<>"']/g,char=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[char]);
  const assetPath=path=>offline?"../"+path:path;
  const facts=Object.entries({"住所":store.facts.address,"電話":store.facts.phone,"サービス提供時間":store.facts.serviceHours,"営業日":store.facts.open,"通常の実施地域":store.facts.area,"定員":store.facts.capacity}).map(([k,v])=>`<div><dt>${k}</dt><dd>${v}</dd></div>`).join("");
  const menuLinks=`<a href="#support">特徴</a><a href="#movie">動画</a><a href="#fit">合う方</a><a href="#day">1日</a><a href="#price">料金</a><a href="#availability">空き状況</a><a href="#contact">相談</a>`;
  const commonTop=`
